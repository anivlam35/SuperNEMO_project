			...
			for(int j = 0; j<Eve->get_no_tracks(); j++)
			{
				...
				if(Z!=0 && Y_MIN < Y && Y < Y_MAX && Z_MIN < Z && Z < Z_MAX)
				{
					...
					if(Eve->get_tracks()[j]->get_side()==0)
					{
						...
						B_Tree[0][NSOR] = Eve->get_tracks()[j]->get_b();
						C_Tree[0][NSOR] = Eve->get_tracks()[j]->get_c();
						D_Tree[0][NSOR] = Eve->get_tracks()[j]->get_d();
					...
					{
						A_Tree[1][NSOR] = Eve->get_tracks()[j]->get_a();
						B_Tree[1][NSOR] = Eve->get_tracks()[j]->get_b();
						C_Tree[1][NSOR] = Eve->get_tracks()[j]->get_c();
						D_Tree[1][NSOR] = Eve->get_tracks()[j]->get_d();
