...
int event_selection(TKEvent* event, double low_qlt, double high_qlt, int ambg_mode)
{
	...
	if(event->get_no_tracks() == 0)
	{
		choice = 0;
	        ...
	        event->get_track(0)->get_quality() >  low_qlt && event->get_track(0)->get_quality() < high_qlt)
	{
		choice = 1;
	}
	else if(event->get_no_tracks() == 2 && event->get_track(0)->get_ambiguity_type() != 0 && event->get_track(0)->get_quality() > low_qlt && event->get_track(0)->get_quality() < high_qlt)
	{
		switch(ambg_mode)
		{
		case 0:
